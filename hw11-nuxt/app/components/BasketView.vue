<template>
  ...
  <div v-if="itemIdCount != 0" class="basket__list">
    ...
    <template class="basket__row" v-for="[id, count] in Object.entries(basket)">
      ...
      :count="String(count)"
      :catalog="catalog"
      @delFromBasket="emit('del-from-basket', id)"
      @changeCount="emit('change-count', $event)"
      @addToBasket="emit('add-to-basket', id)"></BasketLine>
    </template>
...
import {type Product, type Info, type Basket} from './subcomponents.types.ts'
...
const emit = defineEmits<{
  (e: 'del-from-basket', id: string): void;
