<template>
      ...
      <button class="basket__point basket__point--button"
        @click="emit('del-from-basket')">-</button>
      <input class="basket__point basket__point--input" 
        type="number"
        :value="count"
        @blur="emit('change-count', {count: ($event?.target as HTMLInputElement)?.value, id: id})"
        @keyup.enter="emit('change-count', {count: ($event?.target as HTMLInputElement)?.value, id: id})"></input>
      <button class="basket__point basket__point--button"
        @click="emit('add-to-basket')">+</button>
      <span class="basket__point basket__point--right">{{ Number(price).toFixed(2)}}</span>
      <span class="basket__point basket__point--right">{{ Number(price).toFixed(2)}}</span>
</template>

<script setup lang="ts">
import { type Product, type Info } from './types.ts' 
