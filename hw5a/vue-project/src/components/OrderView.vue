<template>
  ...
  <div>
    ...
    <div class="order">
      ...
      <template v-for="[id, count] in Object.entries(obj.basket.value)">
        <span class="order__product-title">{{ getTitle(id)}}</span> <span> {{ count }} x {{ getPrice(id)}} <b>$</b></span> <span> = {{ obj.itemCost(id)}} <b>$</b></span>
      </template>
...
<script setup>
import { privateInfo, isFilled, clearPrivateInfo } from '@/comp-store/user-info';
import * as obj from '@/comp-store/basket';
import { getTitle, getPrice } from '@/comp-store/catalog';
...
function toOrder() {
  if(isFilled && (obj.itemIdCount.value > 0)){
