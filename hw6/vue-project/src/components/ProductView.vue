  ...
  <div class="product-card">
    ...
    <div class="product-card__inner">
      ...
      <div class="product-card__price-wrapper">
          <span class="product-card__price">  price: {{Number(product.price).toFixed(2) }} $</span>
          <span class="product-card__category"> {{ product.category }}</span>
      </div>
      <div class="product-card__buttons">
        <button @click="obj.addToBasket(product.id, 1)" class="product-card__button">add to basket</button>
        <router-link :to="{name: 'basket'}" class="product-card__button ">go to basket</router-link>
      </div>
      <div  class="product-card__basket-info">
        <span class="product-card__basket-count">already in basket: {{ obj.itemCount(product.id) }}</span>
        <span class="product-card__basket-cost">cost: {{  obj.itemCost(product.id)  }}</span>
      </div>
...
const product = reactive(cat.getItem(route.params.id));
</script>
